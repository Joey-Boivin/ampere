...
const struct wm_actions* amp_layout_add_window(struct amp_wm_layout* layout, unsigned long focused_window_id, unsigned long window_to_add_id)
{
...
static void _add_first_window(struct amp_wm_layout* layout, unsigned long window_to_add_id)
{
    ...
    new_root->left = NULL;
    new_root->right = NULL;
    struct window_node* base = (struct window_node*)new_root;
    base->next = malloc(sizeof(*new_root->node->next));
    if (!base->next)
    {
        AMP_LOGGER_ERROR("Dynamic memory allocation error");
        exit(EXIT_FAILURE);
    ...
    base->next->win.window_id = window_to_add_id;

    base->win.width = layout->monitor_width;
    base->win.height = layout->monitor_height;
    base->win.x_position = 0;
    base->win.y_position = 0;
...
struct amp_wm_layout* amp_layout_init(unsigned monitor_width, unsigned monitor_height)
{
    ...
    layout->ids = calloc(INITIAL_NUMBER_OF_COLUMNS, sizeof(*layout->ids));
    if (!layout->ids)
    {
        AMP_LOGGER_ERROR("Dynamic memory allocation error");
        free(layout);
    ...
    for (size_t column_index = 0; column_index < INITIAL_NUMBER_OF_COLUMNS; ++column_index)
    {
        unsigned long* column = layout->ids[column_index];
        for (size_t row_index = 0; row_index < INITIAL_NUMBER_OF_ROWS; ++row_index)
        {
            column = calloc(INITIAL_NUMBER_OF_ROWS, sizeof(**layout->ids));
            if (!column)
            {
                AMP_LOGGER_ERROR("Dynamic memory allocation error");
                exit(EXIT_FAILURE);
